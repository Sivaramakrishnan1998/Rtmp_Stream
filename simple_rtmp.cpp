...
cv::VideoCapture get_device(int camID, double width, double height)
{
  ...
  if (!cam.isOpened())
  {
    ...
    std::cout << "video capture device is on \n\n\n";

    std::cout<< cv::CAP_PROP_FRAME_WIDTH << "  "<<  cv::CAP_PROP_FRAME_HEIGHT << '\n';
...
void initialize_codec_stream(AVStream *&stream, AVCodecContext *&codec_ctx, AVCodec *&codec,const char * codec_profile)
{
...
AVFrame *allocate_frame_buffer(AVCodecContext *codec_ctx, double width, double height)
{
    ...
    //av_image_alloc(frame->data, frame->linesize,width, height , codec_ctx->pix_fmt,  1);
...
void write_frame(AVCodecContext *codec_ctx, AVFormatContext *fmt_ctx, AVFrame *frame)
{
    ...
    if(codec_ctx!= nullptr) std::cout << "codec_ctx not null  ";
...
void stream_video(double width, double height, int fps, int camID, int bitrate, const char* codec_profile,const char * server)
{
    ...
    const char *output = nullptr;//server.c_str();
...
{ 
    std::cout << "parameters is "<< "capture-device-id fps width height bitrate\n use ctrl + c to stop theststream\n";  

    int cameraID = 2, fps = 25, width = 640, height = 480, bitrate = 300000;

    const char * h264profile = "main";
    //std::string
    const char * outputServer = "rtmp://localhost/live/stream";
